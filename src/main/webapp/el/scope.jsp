...
<body>
	...
	<!-- el scope 확인 예제  -->
	<!--	  
		page(pageContext) : jsp 페이지 에서만 유효한 속성
		request	: 요청이 끝날떄 까지 유효한 속성
		session	: session이 만료 되기 전까지 유효한 속성(세션이 만료되는 시점? : server 종료시  / tomcat 설정에 따라
														  session.invalidate() /
														  사용자가 server timeout 시간안에 요청을 하지 않았을때) 
